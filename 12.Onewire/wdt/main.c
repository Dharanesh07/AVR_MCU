...
volatile bool deep_sleep = 0;
volatile uint8_t wdt_counter = 0; // Counter to track WDT interrupts
...
ISR(WDT_vect) {
    wdt_counter++; // Increment the counter every 1 second

    if (wdt_counter >= 5 && deep_sleep) { // Check if 5 seconds have elapsed
