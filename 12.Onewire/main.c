...
void tasks()
{
        ...
        */ 
        ...
        {
            max_temp = (curr_temp > max_temp) ? curr_temp : max_temp;
            min_temp = (curr_temp < min_temp) ? curr_temp : min_temp;
        }
        /*
         if(is_updated){
...
ISR(TIMER2_COMPA_vect) {
    ...
    if(global_time == ON_TIME_BP_MS)
    {
       ...
       //printf("checking every 5 sec...\t value = %f\n",bp103_light_next);
        ...
        {
            is_day=1; // change to 0 for night detection 
            global_time = 0;
            wakeup_flag = 1; // change to 0 for sleep
        }
        else 
        {
            is_day = 1;
